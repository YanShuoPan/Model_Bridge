...
from backend.services.ai_service import analyze_question_with_gpt
...
def _simple_task_from_question(q: str, df_summary: dict = None, use_gpt: bool = True) -> str:
    """
    識別任務類型，優先使用 GPT，失敗時使用關鍵字匹配

    Args:
        q: 使用者問題
        df_summary: 數據摘要（用於 GPT 分析）
        use_gpt: 是否使用 GPT（預設為 True）

    Returns:
        任務類型字串
    """
    # 嘗試使用 GPT 分析
    if use_gpt and df_summary:
        try:
            gpt_result = analyze_question_with_gpt(q, df_summary)
            if gpt_result.get("confidence") in ["high", "medium"]:
                task_type = gpt_result.get("task_type", "prediction")
                print(f"[GPT] 任務識別: {task_type} ({gpt_result.get('reasoning')})")
                return task_type
        except Exception as e:
            print(f"[GPT] 分析失敗，使用關鍵字匹配: {e}")
...
def parse_question_and_csv(question, file):
    ...
    # 使用 GPT 進行任務識別
    task = _simple_task_from_question(question, df_summary=df_summary, use_gpt=True)
