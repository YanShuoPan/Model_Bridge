...
AVAILABLE_METHODS = {
    ...
    "oga_hdic": {
        ...
        "suitable_for": ["高維度預測", "變數篩選", "基因體學分析", "特徵選擇"],
        "example_questions": [
            "我有幾千個基因表達量，想找出與疾病相關的關鍵基因？",
            "如何從數百個經濟指標中選出預測GDP的重要變數？",
            "在大量感測器數據中，哪些對預測故障最重要？"
        ],
        "assumptions": [
            "真實模型是稀疏的（只有少數變數真正重要）",
            "樣本間相互獨立",
            "線性關係假設（結果變數與選出的變數呈線性關係）"
        ],
        "outputs": [
            "選擇的重要變數列表",
            "迴歸係數與顯著性",
        ...
        "example_data": {
            "name": "highdim_demo.csv",
            "path": "backend/storage/demo/highdim_demo.csv",
            "description": "模擬的高維度數據集（變數數 >> 樣本數）",
            "columns": {
                "y": "連續型結果變數",
                "x1, x2, ..., x100": "大量預測變數（模擬基因、感測器、經濟指標等）"
            },
            "sample_size": "約 50-100 筆（變數遠多於樣本）",
            "what_to_expect": "可以看到從大量變數中篩選出的重要變數、HDIC曲線顯示選擇過程、以及最終模型的預測表現"
        }
    }
}
